...
| **C — Knowledge module** | ✅ PARTIAL 2026-06-11 — knowledge collections (create/list/delete), paragraph-aware chunker, document ingestion (embed-on-write) + semantic search reusing the pgvector long-term store; builder Knowledge page with ingest + retrieval test. Secure-stack postgres switched to pgvector image. Remaining: file/PDF loaders, citation surfacing in the run console, rerank. | partial | Open WebUI (port), Dify (design) |
