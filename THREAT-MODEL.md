...
|-----------|------------|--------|----------------|
| T1 | Central route inventory with middleware enforcement; startup validation catches unclassified endpoints; secure/local full-stack fails closed via env-backed auth defaults | **Implemented** | \pps/backend/app/request_security.py\, \pps/backend/app/main.py\ route classification, \FRONTIER_RUNTIME_PROFILE=local-secure\ |
| T2 | Structured logging redacts auth headers, tokens, secrets; error responses sanitize guardrail internals; no PII in traces by default | **Implemented** | \pps/backend/app/logging_config.py\, runtime sanitization in \rontier_runtime/security.py\ |
| T3 | Prepared statements / safe query builders in backend; graph schema validation; input length/format checks at API boundary | **Implemented** | SQLAlchemy ORM, Pydantic models in \pps/backend/app/schemas.py\ |
| T4 | Canonical containment checks (not prefix matching) in backend loaders and shared fallback policy; symlink resolution before containment test | **Implemented** | \pps/backend/app/filesystem.py\, \policies/fs.rego\ |
| T5 | Squid domain allowlist (fail-closed) replacing open IP-range ACL; network namespace isolation when \llow_network=False\ | **Implemented** | \docker/sandbox/squid.conf\, \rontier_runtime/sandbox.py\ |
| T6 | A2A replay protection: TTL-based nonce expiry with bounded pruning; signed \X-Frontier-Subject/Nonce/Signature\ headers verified at receiver | **Implemented** | \pps/backend/app/a2a_replay.py\, worker A2A client in \pps/workers/\ |
| T7 | Capability tokens carry \xp\ (10-min TTL), \llowed_tools\, \max_tool_calls\, canonical read/write path scopes; shared verifier enforces all claims | **Implemented** | \rontier_runtime/capability_tokens.py\, filter-chain enforcement |
| T8 | Memory reads/writes enforce actor, tenant, collaboration-session, or internal-service authorization per scope; scope-to-bucket validation | **Implemented** | \pps/backend/app/memory.py\, worker runtime envelope auth middleware |
| T9 | Normalized evaluation contract shared by Python fallback and Rego; parity tests for path access, dynamic tool allowlists, tool budgets | **Implemented** | \policies/agent.rego\, \rontier_runtime/policy_fallback.py\, \	ests/unit/test_*_parity.py\ |
| T10 | Central route classification marks \/healthz\ public (minimal), diagnostics authenticated; secure local mode serves minimal public healthz | **Implemented** | \pps/backend/app/request_security.py\ access classes |
| T11 | CORS uses explicit local origins/methods/headers instead of wildcards; localhost-only in local profiles | **Implemented** | \pps/backend/app/main.py\ CORS middleware config |
...
| T13 | Three-tier sandbox: seccomp BPF, read-only rootfs, network namespace, non-root, resource limits, IPC isolation, gVisor/Kata RuntimeClasses for K8s | **Implemented** | \docker/sandbox/seccomp-strict.json\, \rontier_runtime/sandbox.py\, Helm chart \rontier-sandbox\ RuntimeClass |
| T14 | Bounded pids-limit (256), memory (512m), CPU (1.0); capability token \max_tool_calls\ budget; sandbox resource limits | **Implemented** | \rontier_runtime/sandbox.py\, capability token claims |
| T15 | Explicit \FRONTIER_RUNTIME_PROFILE\ values (\local-lightweight\, \local-secure\, \hosted\); Helm values-prod.yaml codifies hosted posture; CI validates Helm render | **Implemented** | \rontier_tooling/common.py\, \helm/lattix-frontier/values-prod.yaml\, CI Helm lint |
