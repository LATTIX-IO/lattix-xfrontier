...
- `AGENTS.md` — full agent guidance, including the shared Lattix engineering standard
- `.github/instructions/*.instructions.md` — path-scoped rules applied automatically
- `.github/prompts/*.prompt.md` — shared task prompts
- `QUALITY_SCORE.md`, `RELIABILITY.md`, `SECURITY.md`, `WORKFLOW.md` — repo standards

## Build / test
- `make test`, `make lint`, `make typecheck`, `make policy-test`
- Frontend: `npm run test`, `npm run lint`, `npm run build` (in `apps/frontend/`)
- `apps/backend/` is not covered by `make typecheck`; `ruff format --check` is not in CI. Treat a green gate as partial evidence.
...
## Security
Never store or commit credentials, tokens, private keys, or customer/regulated data. Record only that a secret exists and where it is retrieved through approved secret management. Full controls and trust boundaries: `SECURITY.md` and `THREAT-MODEL.md`.
